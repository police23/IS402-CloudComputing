...
const getAddressesByUserID = async (req, res) => {
    try {
        ...
        res.status(error.status || 500).json({
            success: false,
            message: error.message || 'Có lỗi xảy ra khi lấy danh sách địa chỉ'
        });
    }
};
...
const addAddress = async (req, res) => {
    try {
        ...
        res.status(error.status || 500).json({
            success: false,
            message: error.message || 'Có lỗi xảy ra khi thêm địa chỉ'
        });
    }
};
...
const deleteAddress = async (req, res) => {
    try {
        ...
        res.json({
            success: true,
            message: result.message || 'Xóa địa chỉ thành công'
        });
    } catch (error) {
...
const setDefaultAddress = async (req, res) => {
    try {
        ...
        res.status(error.status || 500).json({
            success: false,
            message: error.message || 'Có lỗi xảy ra khi đặt địa chỉ mặc định'
        });
    }
};
